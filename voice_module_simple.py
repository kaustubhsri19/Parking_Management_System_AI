    ...
    def get_voice_input(self, timeout: int = 5, phrase_time_limit: int = 10) -> Tuple[bool, str]:
        ...
        try:
            if not self.microphone_available:
                # Return error instead of hardcoded value
                logger.warning("Microphone not available - cannot capture voice input")
                return False, "Microphone not available. Please use the text input or enable microphone permissions."
            ...
            # Return error instead of hardcoded demo command
            error_msg = f"Voice recognition failed: {str(e)}"
